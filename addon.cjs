// addon.cjs – universal e estável 23 junho + integração com catálogo oficial Stremio
...
async function getTitleByExternalId(externalId) {
    try {
        ...
        if (imdbMatch) {
            ...
            const tmdbType = parts.length > 1 ? parts[0] : 'movie';
...
const addon = {
    ...
    async getCatalog(type, id, extra, configBase64) {
        ...
        const lIdx = parseInt(id.split('_')[1]) || 0;
    ...
    async searchCatalog(type, query, lists, skip = 0) {
        ...
        for (let i = 0; i < lists.length; i++) {
            ...
            if (!config || config.type === 'm3u') continue;
            ...
            // Fallback local: percorre as primeiras 3 páginas
            if (!items.length) {
                for (let page = 0; page < 3; page++) {
                    try {
                        const pageUrl = `${apiBase}&type=${sType}&action=get_ordered_list&p=${page}`;
                        const r = await axios.get(pageUrl, opts);
                        const data = r.data?.js?.data || r.data?.js || [];
                        const pageItems = Array.isArray(data) ? data : Object.values(data);
    ...
    async getMeta(type, id, configBase64) {
        ...
        if (parts.length < 3 || !parts[1].includes('_')) return { meta: {} };
        ...
        const listSig = config ? crypto.createHash('md5').update(config.url).digest('hex').substring(0,4) : "";
    ...
    async getStreams(type, id, configBase64, host) {
        ...
        // Se for ID externo (catálogo oficial), faz a ponte
        if (!id.startsWith('xlv:')) {
            ...
            const titleInfo = await getTitleByExternalId(cleanId);
                ...
                for (let listIdx = 0; listIdx < lists.length; listIdx++) {
                    ...
                    for (let page = 0; page < 5; page++) {
                        try {
                            ...
                            foundItem = items.find(item => normalize(item.name || item.title || '').includes(targetName));
                            if (foundItem) break;
                        } catch(e) {}
                    }
                    ...
                    if (foundItem) {
                        ...
                        if (cmdUrl) {
                            ...
                            const streamTitle = type === 'movie' ? '🎬 Directo Filme' : `🍿 Directo Série - ${titleInfo.name}${season ? ` S${season}` : ''}${episode ? ` E${episode}` : ''}`;
        ...
        // IDs internos (xlv:...)
        const parts = id.split(":");
        if (parts.length < 3 || !parts[1].includes('_')) return { streams: [] };
        const lIdxParts = parts[1].split("_");
        const lIdx = parseInt(lIdxParts[0]);
        const sig = lIdxParts[1];
        const sId = parts[2];
        ...
        if (config.type === 'xtream') {
            const b = config.url.trim().replace(/\/$/, "");
            if (type === 'tv') streams.push({ name: name, url: `${b}/${config.user}/${config.pass}/${sId}`, title: '📺 Directo TV', behaviorHints: { notWebReady: true }, contentType: 'video/mp2t' });
            else if (type === 'movie') streams.push({ name: name, url: `${b}/movie/${config.user}/${config.pass}/${sId}`, title: '🎬 Directo Filme', behaviorHints: { notWebReady: false } });
            else if (type === 'series') streams.push({ name: name, url: `${b}/series/${config.user}/${config.pass}/${sId}`, title: `🍿 Directo Série - ${name}`, behaviorHints: { notWebReady: false } });
                ...
                if (auth) {
                    ...
                    if (cmdUrl) {
                        ...
                        if (cleanUrl.includes('://')) {
                            const titleStr = type === 'movie' ? '🎬 Directo Filme' : `⚡ Directo TV`;
            ...
            if (!directAdded) {
                const fallbackUrl = decodeURIComponent(sId).split('|||')[0].split('|')[0].replace(/^(ffrt|ffmpeg|ffrt2|rtmp)\s+/, "").trim();
                if (fallbackUrl.startsWith('http')) {
                    streams.push({ name: name, url: fallbackUrl, title: 'Directo TV', behaviorHints: { notWebReady: true }, contentType: 'video/mp2t' });
                }
            }
        }
