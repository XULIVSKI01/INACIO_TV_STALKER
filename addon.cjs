// addon.cjs – universal e estável 23 junho (corrigido)
                ...
                if (l.type === 'xtream') {
                    ...
                    if (auth) {
                        ...
                        const [g1, g2, g3] = await Promise.all([
                            fetchSt('itv', 'get_categories', 'get_genres'),
    ...
    async getCatalog(type, id, extra, configBase64) {
        ...
        try {
            ...
            if (config.type === 'xtream') {
                ...
                const page = Math.floor(skip / 30) + 1; // assumimos 30 itens por página
                let res;
                try {
    ...
    async searchCatalog(type, query, lists, skip = 0) {
        ...
        for (let i = 0; i < lists.length; i++) {
            ...
            if (config.type === 'm3u') {
                try {
                    const channels = await parseM3U(config.m3uUrl, config);
                    const filtered = channels.filter(c => normalize(c.name).includes(searchQuery));
                    filtered.forEach(c => {
                        allMetas.push({
                            id: `xlv:${i}_${listSig}:${encodeURIComponent(c.url)}:${encodeURIComponent(c.name)}:${encodeURIComponent(c.logo || '')}`,
                            name: c.name,
                            type: 'tv',
                            poster: c.logo,
                            posterShape: 'landscape'
                ...
                } catch(e) { console.warn(`[SEARCH M3U] Erro: ${e.message}`); }
                continue;
            }
            ...
            if (config.type === 'xtream') {
                try {
                    ...
                    let searchUrl = `${api}&action=${action}&search=${encodeURIComponent(query)}`;
                    let res = await axios.get(searchUrl, this.getAxiosOpts(config, { timeout: 10000 }));
                    let items = Array.isArray(res.data) ? res.data : [];

                    if (items.length === 0) {
                        const allUrl = `${api}&action=${action}`;
                        const allRes = await axios.get(allUrl, this.getAxiosOpts(config, { timeout: 15000 }));
                        const allItems = Array.isArray(allRes.data) ? allRes.data : [];
                        items = allItems.filter(item => normalize(item.name || item.title || '').includes(searchQuery));
                    }

                    items.slice(0, 100).forEach(item => {
                        const targetId = item.stream_id || item.series_id || item.id || '';
                ...
                } catch(e) { console.warn(`[SEARCH Xtream] Erro: ${e.message}`); }
                continue;
            }
            ...
            if (config.type === 'stalker') {
                try {
                    ...
                    // Fallback local (percorre até 10 páginas)
                    if (items.length === 0) {
                    ...
                    items.forEach(item => {
                        const targetId = item.id || item.cmd || '';
                        const name = item.name || item.title || '';
                ...
                } catch(e) { console.warn(`[SEARCH Stalker] Erro: ${e.message}`); }
            }
        }
